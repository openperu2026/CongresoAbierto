...
def process_bancada(raw_bancada: RawBancada) -> tuple[list[Bancada], list[BancadaMembership]]:
    """
    ...
    for row in rows:
        ...
        if len(childs) == 1:
            ...
            if raw_bancada.legislative_period != current_leg_period:
                ...
                leg_year = LegislativeYear(str(int(raw_bancada.legislative_period[-4:])-1))
            
            bancadas_lst.append(Bancada(
                leg_year = leg_year,
