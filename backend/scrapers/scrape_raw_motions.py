...
class RawMotionScraper:
    ...
    def scrape_motion(self, year: str, motion_number: str) -> None:
        ...
        try:
            ...
            logger.success(f"Successfully scraped Raw Bill {year}_{motion_number}")
    ...
    def add_motions_to_db(self) -> bool:
        ...
        try:
            ...
            session.commit()
            logger.success(f"Added {len(self.raw_motions)} Raw Bills to table.")
            return True

        except SQLAlchemyError as e:
            logger.error(f"Failed to add motions to Raw Bills table: {e}")
            session.rollback()
            return False
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if len(scraper.raw_motions) % 100 == 0:
