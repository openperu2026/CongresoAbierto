...
from estecon.backend.scrapers.scrape_utils import get_url_text
...
class RawBillScraper:
    ...
    def create_raw_bill(self, year: str, bill_number: str, data: dict) -> RawBill:
        ...
        for raw_name, attribute_name in self.section_mapping.items():
            ...
            if attribute_value == "Not Found":
                logger.warning(f"{raw_bill.id} - Missing Attribute: {raw_name} ({attribute_name})")
            else:
    ...
    def add_bills_to_db(self) -> bool:
        ...
        """
        assert len(self.raw_bills) != 0, "There are no Raw Bills scraped. Nothing to load to DB."
